...
// заменить на SPIFFS_OBJ_NAME_LEN из spiffs_config.h
...
typedef char Ssid[WIFI_CONFIG_SSID_MAX_LENGTH];
...
public:
	...
	//bool getStoredNetworksList(Ssid* ssids[WIFI_CONFIG_NETWORKS_AMOUNT]);
	//bool getStoredNetworksList(String *ssids[WIFI_CONFIG_NETWORKS_AMOUNT]);
	bool getCurrentNetwork(const Ssid* ssid);
	//bool networkExsists(const Ssid* ssid);
