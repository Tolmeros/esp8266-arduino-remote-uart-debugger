...
/*
bool JsonSettingsWiFi::getStoredNetworksList(Ssid *ssids[WIFI_CONFIG_NETWORKS_AMOUNT]) {

	for (byte i=0; i++; i<networks_count) {
		strlcpy(ssids[i], networks[i].ssid, sizeof(ssids[i]));
	}
...
/*
bool JsonSettingsWiFi::getStoredNetworksList(String *ssids[WIFI_CONFIG_NETWORKS_AMOUNT]) {

	for (byte i=0; i++; i<networks_count) {
		//strlcpy(ssids[i], networks[i].ssid, sizeof(ssids[i]));
		*ssids[i] = String(networks[i].ssid);
	}
